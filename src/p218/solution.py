    ...
    def getSkyline(self, buildings):
        ...
        for l, r, h in buildings:
            if l == buildings[0][0]:
                ...
                points.append(Point(l, 0))
            points.append(Point(l, h))
            if r == max_x:
                points.append(Point(r, -1))
            else:
                points.append(Point(r, 0))
            points.append(Point(r, h))

        convex_hull = ConvexHull(points).convex_hull
        result = []
        for i in range(1, len(convex_hull)):
            if convex_hull[i].y == convex_hull[i - 1].y and convex_hull.y != -1:
                result.append([convex_hull[i - 1].x, convex_hull[i - 1].y])
